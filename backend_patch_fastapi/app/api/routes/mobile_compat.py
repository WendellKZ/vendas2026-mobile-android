...
class PedidoPayload(BaseModel):
    ...
    acao: str | None = None
    status_solicitado: str | None = None
    manter_orcamento: bool | None = None
    itens: list[PedidoItemPayload] = []


def _status_from_mobile_payload(payload: PedidoPayload) -> str:
    requested = " ".join(
        str(value or "")
        for value in [
            payload.tipo_finalizacao,
            payload.acao,
            payload.status_solicitado,
        ]
    ).upper()
    if payload.manter_orcamento is True or "ORCAMENTO" in requested or "ORÇAMENTO" in requested:
        return "em_orcamento"
    if "APROVACAO" in requested or "APROVAÇÃO" in requested or "EM_APROVACAO" in requested:
        return "em_aprovacao"
    return "em_orcamento"
...
def criar_pedido(payload: PedidoPayload, db: Session = Depends(get_db), user: User = Depends(current_mobile_user)):
    ...
    if not customer:
        customer = Customer(company_id=empresa_id, name=payload.nome_cliente or "Cliente padrão mobile", document="", city="", status="ativo")
        db.add(customer)
        db.flush()
    carrier_id = to_int(payload.codigo_transportadora)

    order_status = _status_from_mobile_payload(payload)
...
@router.get("/config")
def mobile_config(user: User = Depends(current_mobile_user)):
    """Configuração remota simples consumida pelo app Android no botão Sincronizar agora."""
    return {
        "app_version_label": "V40",
        "mensagem_home": "",
        "cor_primaria": "#17385C",
